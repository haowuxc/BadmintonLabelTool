...
from utils_for_imgLabelBat import save_info, load_info, go2frame, show_image
...
def ball_label(event, x, y, flags, param):
    global frame_no, point_no, info, image
    if event == cv2.EVENT_LBUTTONDOWN:
        # h, w, _ = image.shape
        # info[frame_no]['x'] = x/w
        # info[frame_no]['y'] = y/h
        info[frame_no][f'X{point_no}'] = x
        info[frame_no][f'Y{point_no}'] = y
        # info[frame_no][point_no]['Visibility'] = 1

    elif event == cv2.EVENT_MBUTTONDOWN or event == cv2.EVENT_RBUTTONDOWN:
        info[frame_no][f'X{point_no}'] = -1
        info[frame_no][f'Y{point_no}'] = -1
        # info[frame_no][point_no]['Visibility'] = 0

saved_success = False
frame_no = 0
point_no = 0
_, image = cap.read()
show_image(image, 0, 0, info)
while True:
    leave = 'y'
    ...
    if key == ord('e'):
        ...
        frame_no += 1
        point_no = 0
        image = go2frame(cap, frame_no, point_no, info)
        print("Frame No.{}, Point No.{}".format(frame_no, point_no))
    
    elif key == ord('o'): # next point
        if point_no >= n_point-1:
            if frame_no >= n_frames-1:
                print("This is the last frame")
                continue
            frame_no += 1
